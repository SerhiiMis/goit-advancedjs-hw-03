...
`).join(""),u=document.querySelector(".action-form"),l=document.querySelector(".loader"),d=document.querySelector(".gallery");let c="",E=new y(".gallery a",{captionsData:"alt",captionDelay:250});u.addEventListener("submit",O);function O(t){if(t.preventDefault(),c=u.elements.query.value.trim(),d.innerHTML="",l.classList.remove("is-hidden"),c==="")return l.classList.add("is-hidden"),i("Please enter a valid query!");S(c).then(e=>{e.hits.length===0?i("No results found. Please try another search term."):(b(e.hits),E.refresh())}).catch(e=>{console.error("Error fetching images:",e),i("Sorry, something went wrong. Please try again!")}).finally(()=>{l.classList.add("is-hidden")}),u.reset()}
